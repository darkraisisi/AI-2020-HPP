...
#Exercises
#1.
def testSorts(func,length:int,size:int):
    lst = list(np.random.randint(size,size=length))
    sortedList = func(lst)
    return sortedList
...
def testSuite():
    functions = [selection_sort, 
                insertion_sort,
                merge_sort]
    lengths = [1000, 10000, 30000]

    for length in lengths:
        for func in functions:
            print(f'ArrLen: {length}, func: {str(func.__name__)}, Start')
            startTime = time.time()
            testSorts(func,length,100)
            print(f'ArrLen: {length}, func: {str(func.__name__)} Took: {time.time() - startTime}\n')
